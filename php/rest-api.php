...
function qsm_rest_get_bank_questions( WP_REST_Request $request ){
    if(is_user_logged_in()){
        ...
        $quiz_query = '';
        if($quizID){
            $quiz_query = ' AND quiz_id = "' . $quizID . '"';
        }
        $total_count_query = $wpdb->get_row( "SELECT COUNT(question_id) as total_question FROM {$wpdb->prefix}mlw_questions WHERE deleted='0' AND deleted_question_bank='0'$category_query $quiz_query", 'ARRAY_A' );
